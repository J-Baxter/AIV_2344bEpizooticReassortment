  ...
  if(nrow(temp[[i]])>0){
    ...
    new_meta_formatted[[i]] <- temp[[i]] %>%
      ...
      mutate(date_format = case_when(
        ...
        grepl('\\d{2}-\\d{2}-\\d{4}', date)  ~ "dd-mm-yyyy", 
...
test_nodups <- RemoveDuplicated(alignments[[1]])

matches <- unlist(lapply(rownames(test_nodups), function(x) regmatches(x, gregexpr("EPI_ISL_\\d+", x))))


duplicated_indices <- which(duplicated(matches) | duplicated(matches, fromLast = TRUE))
duplicated_items <- matches[duplicated_indices]
duplicated_list <- sapply(duplicated_items, function(x) which(grepl(x, rownames(alignments[[1]]))), simplify = F)
duplicated_seqnames <- lapply(duplicated_list, function(x) rownames(alignments[[1]])[x])
