...
FormatMetadata <- function(data){
  ...
  data <- data %>%
    ...
    mutate(primary_com_name = case_when(
      ...
      grepl('grey goose|graylag goose|grey go*|grey lag*', source) ~ 'greylag goose',
      grepl("shoveler", source) ~ "northern shoveler",
      grepl("white-fronted goose", source) ~ "greater/lesser white-fronted goose",
      grepl("mallard duck|^mal$", source) ~ "mallard",
      grepl("chicken|ckicken|layer hen|laying hen|broiler|poultry|chichen|^hen$|^rooster$|^layer$|^chiken$|^ch$", source) ~ "red junglefowl (domestic type)",
      ...
      grepl("domestic duck|pekin duck|mule duck|runner duck|cascade duck|rural duck|mulard duck", source) ~ "mallard (domestic type)",
     ...
     # grepl("pink footed goose", source) ~ "pink-footed goose",
      ...
      grepl("african fish eagle|^fish eagle$", source) ~ "african fish-eagle",
      ...
      grepl("blac[kh][- ]headea{0,1}d gull|bl h gull", source) ~ "black-headed gull",
      ...
      grepl("black-swan|bk swan", source) ~ "black swan",
      ...
      grepl('mute[- ]{0,1}swan|cignus olor|^m swan', source) ~"mute swan",
      grepl('^rhea$', source)  ~ "rhea sp.",
      grepl('bewicks{0,1} swan', source) ~ 'tundra swan',
      grepl('gr bk bd gull|great black backed gull', source) ~ 'great black-backed gull',
      grepl('sacred ibis', source) ~ 'african sacred ibis',
      grepl('withe-tiled{0,1} eagle', source) ~ 'white-tailed eagle',
      grepl("belchers{0,1} gull", source) ~ "belcher's gull",
      grepl("white fronted goose", source) ~ "greater/lesser white-fronted goose",
      grepl('lesser auk', source) ~ "razorbill",
      
      grepl('blue[ -]winged[ -]teal', source) ~ "blue-winged teal",
      grepl('mergus|merganser', source) ~ "merganser sp.",
      grepl('redhead duck', source) ~ "redhead",
      grepl('black[ -]crowned[ -]night[ -]heron', source) ~ "black-crowned night heron",
      ...
      grepl("branta leucopsis|barnacle", source) ~ "barnacle goose",
      ...
      grepl('ichth{0,1}yaetus melanocephalus', source) ~ "mediterranean gull",
      grepl('anser cygnoides', source) ~ "swan goose",
      grepl('arenaria interpres', source) ~ "ruddy turnstone",
      ...
      grepl("^goose$|wild geese|wild goose|^geese$|^go$", source) ~ "goose sp.",
      ...
      grepl('oyster catcher', source) ~ "oystercatcher sp.",
      grepl('guillemot', source) ~ "alcid sp.",
      
      grepl("shorebird|sandpiper", source) ~ "shorebird sp.",
      grepl("black-backed gull|^gull$|seagull|seabird|^gull*|l-bl-ba-gull|c gull", source) ~ "gull sp.",
      grepl("^crow$|^raven$|american raven", source) ~ "crow/raven sp.",
      grepl("^duck$|wild duck|migratory duck|^dk$", source) ~ "duck sp.",
      ...
      grepl("^pigeon$|^dove$", source) ~ "pigeon/dove sp.",
      grepl("^teal$", source) ~ "teal sp.",
      grepl('^ibis$', source) ~ 'ibis sp.',
      grepl('^rhea$', source) ~ 'rhea sp.',
      grepl('^bulbul$', source) ~ "pycnonotidae sp.",
      grepl('^tern$', source) ~ 'tern sp.',
      grepl('^parrot$', source) ~ "parrot sp.",
      
      grepl('^gannet$', source) ~ "sulid sp.",
      grepl("^heron$", source) ~"heron sp.",
      grepl("^aquatic bird$|^avian$|^bird$|wild[- ]{0,1}birds{0,1}|backyard bird|ornamental bird|breeding hen", source) ~ "bird sp.",
      source %in% c("en", "env", "environment", "enviroment", "environmental", 'environment sample',
                    "water", "wild bird feces") ~ "environment",
      source %in% c("t dk", 'an', 'unknown') ~ "NA",
      is.na(source) ~ 'NA',
      
      # Mammals,
      grepl("red fox|^fox$|vulpes vulpes|blue fox|silver fox", source) ~ "red fox",
      ...
      grepl('^otter$|lutralutra', source) ~ 'otter sp.',
      grepl('^lion$|^cat$|domestic cat|feline|lion', source) ~ 'feline sp.',
      grepl('canine', source) ~ 'canine sp.',
      grepl("arctic[ -]fox|vulpes lagopus", source) ~ "arctic fox",
