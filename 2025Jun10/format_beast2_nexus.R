...
WriteBEAST2Nexus <- function(aln, filename, metadata, SRD06 = TRUE){
  ...
  nexus_dates <- metadata_in_aln %>%
    select(tipnames, starts_with('collection_date'), collection_tipdate) %>%
    
    mutate(date_format = case_when(str_count(collection_tipdate, pattern = '-') == 0 ~ 'yyyy',
                                   str_count(collection_tipdate, pattern = '-') == 1 ~ 'yyyy-mm',
                                   str_count(collection_tipdate, pattern = '-') == 2 ~ 'yyyy-mm-dd',
    ...
    mutate(date_dec = if_else(!is.na(collection_date), decimal_date(collection_date), NA)) %>%
    
    # for year-month and year only, date_dec = start
    mutate(date_lower = case_when(date_format == 'yyyy' ~ decimal_date(as_date(paste0(as.character(collection_dateyear), '-01-01'))),
                                  date_format == 'yyyy-mm' ~ decimal_date(as_date(paste0(collection_datemonth, '-01'))),
                                  .default = date_dec)) %>%
...
WriteBEAST2Dat <- function(aln, filename, metadata){
  stopifnot(is.matrix(aln))
